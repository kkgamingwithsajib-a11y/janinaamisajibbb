...
export const InvestorPortalModal: React.FC<InvestorPortalModalProps> = ({
  ...
  // Live micro-yield ticker calculation for visual feedback on overview
  useEffect(() => {
    const deposits = userAccount?.deposits || [];
    if (!isOpen || deposits.length === 0) return;

    const interval = setInterval(() => {
      const activeTotal = deposits.reduce((acc, dep) => acc + (dep.amountUsd || 0), 0);
  ...
  }, [isOpen, userAccount, platformConfig, onUpdateAccount]);
                ...
                {(userAccount?.deposits || []).map((dep) => (
                  <div key={dep.id} className="p-4 flex items-center justify-between hover:bg-slate-900/40">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="font-bold text-white">Deposit #{dep.id}</div>
                        <div className="text-slate-400 text-[11px]">{dep.startDate} • {dep.dailyYieldPercent}% Yield Node</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-emerald-400 font-bold text-sm">+${(dep.amountUsd || 0).toLocaleString()}</span>
                      <span className="text-[10px] text-emerald-500 uppercase block">{dep.status}</span>
                    </div>
                  </div>
                ))}
                ...
                {(userAccount?.withdrawals || []).map((w) => (
                  <div key={w.id} className="p-4 flex items-center justify-between hover:bg-slate-900/40">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="font-bold text-white">Withdrawal #{w.id}</div>
                        <div className="text-slate-400 text-[11px]">{w.date} • {(w.address || '').substring(0, 10)}...</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="text-rose-400 font-bold text-sm">-${(w.amountUsd || 0).toFixed(2)}</span>
                      <span className="text-[10px] text-slate-400 uppercase block">{w.status}</span>
                    </div>
                  </div>
                ))}
              </div>
