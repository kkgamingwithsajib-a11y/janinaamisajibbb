...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleAdminResetPassword = async (user: AdminUserRecord) => {
    if (!adminToken) return;
    if (!window.confirm(`Issue secure password reset for ${user.name} (${user.email})?`)) return;

    try {
      const tempPass = await api.adminResetPassword(adminToken, user.id);
      setPasswordResetSuccess(tempPass);
      showNotification(`Password reset token generated for ${user.name}.`);
      fetchAllAdminData();
    } catch (err: any) {
      showNotification(err.message, 'error');
    }
